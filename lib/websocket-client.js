...
const wsUrls = {
  ...
  livenet_auth: "wss://stream.bybit.com/realtime_private",
  testnet_auth: "wss://stream-testnet.bybit.com/realtime_private"
};
...
module.exports = class WebsocketClient extends EventEmitter {
  ...
  async _connect() {
    try {
      ...
      let urlPath = this.options.livenet ? 'livenet' : 'testnet'
      if(authParams != ""){
        urlPath = urlPath + "_auth"
      }
      const url = wsUrls[urlPath] + authParams;
      console.log(url)
